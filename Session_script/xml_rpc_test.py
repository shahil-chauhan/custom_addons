...
with open('/home/odoo/Downloads/product_template.csv', newline='') as csv_file:
    ...
    for row in csv_file:
        ...
        if excel_row >= 2:
            ...
            product_tmpl_id = models.execute_kw(db, uid, password, 'product.template',
                'search', [[['default_code','=',rec['default_code'].strip()]]])
            vals = {
                'name':rec['name'].strip(),
            ...
            if not product_tmpl_id:
                product_tmpl_id = models.execute_kw(db, uid, password, 'product.template', 'create', [vals])
            else:
                product_tmpl_id = models.execute_kw(db, uid, password, 'product.template', 'write',  [[product_id[0]], vals])
            print("\n\n:::::::::excel_row:::::::::::::::", excel_row)

            # Example of read method to fetch specific field like emails and name of partner
            # Read method default returns Id field.
            product_details = models.execute_kw(db, uid, password, 'product.template', 'read',[product_id],{'fields':['name', 'list_price']})
