...
class ConstructionSite(models.Model):
    ...
    name = fields.Char(string=_("Name"), required=True, store=True, tracking=True)
    reference = fields.Char(
        string=_("Reference"), required=True, store=True, tracking=True
    )
    schedule_date = fields.Datetime(string=_("Schedules Date"))
    display_name = fields.Char(string=_("Display Name"), compute="_compute_name_code")
    ...
    def _compute_name_code(self):
        for name_code in self:
            name_code.display_name = "[{}] {}".format(
                name_code.reference, name_code.name
            )
